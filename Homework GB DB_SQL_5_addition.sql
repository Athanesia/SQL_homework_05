...
CREATE VIEW friends_of_user_1 AS
SELECT u.id, u.firstname, u.lastname
FROM users u
JOIN friend_requests fr ON u.id = fr.initiator_user_id
WHERE fr.target_user_id = 1 AND fr.status = 'approved';
SELECT * FROM friends_of_user_1;
CREATE VIEW messages_involving_user_1 AS
SELECT m.*
FROM messages m
WHERE m.from_user_id = 1 OR m.to_user_id = 1;
SELECT * FROM messages_involving_user_1 ;
CREATE VIEW user_media_with_likes AS
SELECT m.id, m.user_id, m.filename, m.body, COUNT(l.media_id) AS like_count
FROM media m
LEFT JOIN likes l ON m.id = l.media_id
GROUP BY m.id;
SELECT * FROM user_media_with_likes ;
SELECT u.id, u.firstname, u.lastname, COUNT(uc.community_id) AS group_count
FROM users u
